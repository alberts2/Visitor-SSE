...
# Construct the job name
    ...
    -o /storage1/fs1/michael.landis/Active/visitor_sse/output/stdout2/$NAME  \
    -J $NAME \
    -q general \
    -g /soewongsono/visitor_sse \
    -n 1 -M 3GB -R "rusage [mem=3GB] span[hosts=1]" \
    -a 'docker(sswiston/phylo_docker:slim_amd64)' /bin/bash /storage1/fs1/michael.landis/Active/visitor_sse/scripts/inf_cladomigrate_RIS_full_nomigrate.sh
