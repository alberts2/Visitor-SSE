...
args = {
    ...
    'prop_extant_sampled' : 0.001,             # Expected proportion of lineages at stop_time to be sampled in tree
    ...
    'rv_arg'                : {                # loc/scale/shape for param dists
        ...
        'Sample'            : { 'a' : 2 * 10**-2,   'b' : 10**-1   },     # 10 to 100 days, rate of 0.01 to 0.1
        'ProgressInfected'  : { 'a' : 10**-1,   'b' : 10**0    },     # 1 to 10 days,  rate of 0.1 to 1.0
        'VisitDepart'       : { 'a' : 10**-3,   'b' : 10**-2   },     # 100 to 1000 days, rate of 0.01 to 0.1
        'VisitReturn'       : { 'a' : 10**-1,   'b' : 10**0    },     # 1 to 10 days, rate of 0.1 to 1
        'S0'                : { 'loc' : 400000., 'scale' : 100000. }, # 50000 to 250000 ind. in population
        'R2S'               : { 'loc' : 0.0001, 'scale' : 0.0099 },   # 10000 to 100 days, rate of 0.0001 to 0.01
        'Stop_time'         : { 'loc' : 5.0,    'scale' : 5.0  },     # 5 to 5 days, rate of 0
        'nSampled_tips'     : { 'low' : 200.0,  'high' : 500.   },    # subsample samples
...
if my_model.model_stochastic:
    ...
    if nexus_tree_str.count("time") == 0 or sim_stats['total_sampled'][0] < 2:
